<template>
  <q-page id="recordingListPage">
    ...
    <div class="container">
      <h2>Here is a list of all patients past recordings.</h2>
      <div class="container--inner">
        ...
        <div style="margin-top: 20px">
          <q-infinite-scroll @load="onLoad" :offset="50">
            ...
            >
              <router-link
                :to="detail.finalized && `/admin/details/${detail._id}`"
...
const getNotesFromBack = async (page, limit, search = "") => {
  try {
    const res = await axiosApiInstance.get(
      `${SERVER_URL}/private/admin/notes?noteType=${selection.value.toLowerCase()}&page=${page}&limit=${limit}&search=${search}`
    );
    return res.data;
  } catch (error) {
    $q.notify({
